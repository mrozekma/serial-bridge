...
devices = {device['name']: [Node(node['name'], device['name'], node['com port'], node['baud rate'], node.get('byte size', 8), node.get('parity', 'none'), node.get('stop', 1), node['tcp port']) for node in device.get('nodes', [])] for device in config.get('devices', [])}

for deviceName, nodes in devices.items():
