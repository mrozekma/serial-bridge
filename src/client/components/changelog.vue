<template>
	<div class="changelog">
		<div v-for="{ key, seen } in entries" :class="seen ? 'seen' : 'new'">
			...
			<template v-else-if="key === 'layouts'">
				<h1>Node layouts</h1>
				The default way the device view lays out nodes can now be customized. Several layouts are hardcoded in the Serial Bridge configuration file. You can also rearrange the nodes manually and then go to "View" -> "Layout" -> "Save current layout" to save your own layouts. See the <a href="#setup" target="_blank">"Setup" tab</a> on the homepage to customize the layout behavior, including disabling layouts you don't like and switching which one is applied to devices by default.
			</template>
